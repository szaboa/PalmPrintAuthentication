...
PrincipalLineExtractor::PrincipalLineExtractor(){

}

IFeature* PrincipalLineExtractor::doFeatureExtraction(Mat roi){
...
Mat PrincipalLineExtractor::preprocessing(Mat img){
...
Mat PrincipalLineExtractor::normalizeImage(Mat img){
...
Mat PrincipalLineExtractor::locatePrincipalLines(Mat img){
    ...
    // Apply closing morphology operation to fill the holes
	morphologyEx(lines, lines, 1, element);

    // Apply thinning to transform lines into 1 pixel wide edges
...
Mat PrincipalLineExtractor::filterWithConnectedComponentLabeling(Mat img, int componentMinSize){
...
vector<Point> PrincipalLineExtractor::getNeighborPoints(Point currentPixel){
...
bool PrincipalLineExtractor::isForeground(const Mat& img, Point p){
	if (img.at<uchar>(p.x, p.y) == 255) return true;
	
	return false;
}

bool PrincipalLineExtractor::isInsideTheBoundary(int i, int j){
